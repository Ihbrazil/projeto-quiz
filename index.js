...
const questoes = [
  ...
  // Embaralhar as perguntas aleatoriamente (ARRAY.sort())
  const questoesEmbaralhadas = questoes.sort(() => Math.random() - 0.5);


  // Selecionar as 10 primeiras perguntas (ARRAY.slice())
  const questoesSelecionadas = questoesEmbaralhadas.slice(0, 10);

  // Exibir as perguntas selecionadas (ARRAY.forEach)
  let totaisCorretas = 0;

  questoesSelecionadas.forEach((pergunta, index) => {
    console.log(`${index + 1}: ${pergunta.pergunta}`);
        const anoResposta = entradaDados.question('Digite a resposta (ano): ');
        if (anoResposta === pergunta.resposta) {
  ...
  console.log("Pontuação final: " + totaisCorretas);

  if ((totaisCorretas >= 1) && (totaisCorretas <= 3)) {
    console.log ("OH NÃO! Tente mais uma vez.\n");
  }
  else if ((totaisCorretas >= 4) && (totaisCorretas <= 6)) {
    console.log ("BOM TRABALHO! Pratique um pouco mais.\n");
  }
  else if ((totaisCorretas >= 7) && (totaisCorretas <= 9)) {
    console.log ("MUITO BOM! Você acertou a maioria.\n");
  }
  else if (totaisCorretas >= 10) {
    console.log("EXCELENTE! Você é um verdadeiro expert.\n");
  } 
  else {
    console.log("ERROU TUDO! Matricule-se na DevMedia.\n");
  }